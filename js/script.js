...
let timeValue = 3600; // Initial time value
let remainingTime = timeValue; // Track remaining time
let queCount = 0;
let queNumb = 1;
let userScore = 0;
let counter;
...
function initializeQuiz() {
  ...
  queCounter(queNumb);
  startTimer(remainingTime); // Use remainingTime instead of timeValue
  startTimerLine(widthValue);
}

// Reset quiz variables
function resetQuiz() {
  remainingTime = timeValue; // Reset remaining time
  queCount = 0;
  queNumb = 1;
  userScore = 0;
  widthValue = 0;
...
function startTimer(time) {
  counter = setInterval(() => {
    remainingTime = time; // Update remainingTime
